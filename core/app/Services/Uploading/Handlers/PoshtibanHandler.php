<?php
    ...
    class PoshtibanHandler extends Handler {
        private S3Client $s3;
        ...
        public function __construct( BucketEnum $bucket = BucketEnum::DEFAULT ) {
            ...
            $this->s3 = AWS::createClient( 's3',
                [
                    'endpoint'    => "https://ir-west-1.s3.poshtiban.com",
                    'credentials' => [
                        'key'    => env( 'POSHTIBAN_AWS_ACCESS_KEY_ID', '' ),
                        'secret' => env( 'POSHTIBAN_AWS_SECRET_ACCESS_KEY', '' ),
                    ],
                ] );
        }
        ...
        public function url( Resource $resource ): string {
            $baseUrl = "https://%.ir-west-1.s3.poshtiban.com/%";
            $bucket  = Arr::get( $resource->getOptions(), 'bucket', BucketEnum::DEFAULT->value );
